...
class Api:
    def __init__(self, tool: HHApplicantTool):
        ...
        self._contacts_running: bool = False
    ...
    def run_contacts(self, limit: int = 0, to_sheets: bool = False) -> dict:
        ...
        if self._contacts_running:
            return {"status": "error", "message": "Операция уже выполняется"}
        self._contacts_running = True
        ...
        def _worker() -> None:
            ...
            try:
                ...
                self._send_contacts_event("done", {
                    "total": len(enriched),
                ...
                self._send_contacts_event("error", {"message": str(e)})
            finally:
                self._contacts_running = False
        ...
        def _worker() -> None:
            ...
            try:
                ...
                try:
                    ...
                    self._send_apply_event("error", {"message": "Неверные параметры поиска"})
                    return
                ...
                if cancel_event.is_set():
                    self._send_apply_event("cancelled")
                else:
                    self._send_apply_event("done")
            except Exception as e:
                logger.error("apply_vacancies error: %s", e)
                self._send_apply_event("error", {"message": "Ошибка выполнения операции"})
            finally:
