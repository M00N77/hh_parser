...
async function startApply() {
    if (_applyInProgress) {
        showToast('Операция уже выполняется', 'info');
        return;
    }
...
function onApplyEvent(event, data) {
    ...
    if (event === 'done') {
        ...
        showToast('Ошибка: ' + (data.message || 'неизвестная'), 'error');
    }
...
function onContactsEvent(event, data) {
  ...
  if (event === 'done') {
    status.textContent =
      `Готово: ${data.with_contacts} из ${data.total} с контактами. Google Sheets: ${data.sheets}.`;
    ...
    status.textContent = 'Ошибка: ' + (data.message || 'неизвестно');
  }
}
